<x-app-layout>
    <div x-data="{ 
        ...
        resolve(status) {
            ...
            if (this.activeIndex < this.conflicts.length - 1) {
                this.activeIndex++;
            }
        }
        ...
        <div style="background: white; width: 100%; max-width: 1200px; height: 85vh; border-radius: 20px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5); display: flex; flex-direction: column; overflow: hidden;">
            ...
            <div style="flex: 1; display: flex; overflow: hidden;" x-data="{ isProcessing: false }">
                ...
                <div style="width: 280px; border-right: 1px solid #f1f5f9; display: flex; flex-direction: column; background: #fafafa;">
                    <div style="padding: 24px; flex: 1; overflow-y: auto;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 8px;">
                            <template x-for="(conflict, index) in conflicts">
                                <template x-if="resolved[index] === null">
                                <button @click="activeIndex = index" 
                                    :style="activeIndex === index ? 'text-align: left; padding: 12px 16px; background: #ecfdf5; border: 1.5px solid #10b981; border-radius: 12px; position: relative;' : 'text-align: left; padding: 12px 16px; background: white; border: 1px solid #e2e8f0; border-radius: 12px;'">
                ...
                <div style="flex: 1; display: flex; flex-direction: column; background: #ffffff; position: relative;">
                    ...
                    {{-- Processing Overlay --}}
                    <div x-show="isProcessing" style="position: absolute; inset: 0; background: rgba(255,255,255,0.8); z-index: 50; display: flex; align-items: center; justify-content: center; backdrop-filter: blur(2px);">
                    ...
                    <div style="flex: 1; padding: 32px; overflow-y: auto;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 32px;">
                            ...
                            {{-- Existing Side --}}
                            <div :style="resolution === 'existing' ? 'opacity: 1' : 'opacity: 0.8'">
                                <div style="border: 2px solid transparent; border-radius: 16px; overflow: hidden; transition: all 0.3s;"
                                    :style="resolution === 'existing' ? 'border-color: #3b82f6;' : 'border-color: #e2e8f0;'">
                                    ...
                                    <div style="padding: 24px; border-top: 1px solid #f1f5f9; background: #fafafa;">
                                        <button @click="isProcessing = true; setTimeout(() => { resolve('existing'); isProcessing = false; }, 400)" style="width: 100%; padding: 14px; background: white; border: 2px solid #064e3b; border-radius: 12px; color: #064e3b; font-size: 14px; font-weight: 800; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px;">
                            ...
                            {{-- Incoming Side --}}
                            <div :style="resolution === 'incoming' ? 'opacity: 1' : 'opacity: 0.8'">
                                <div style="border: 2px solid transparent; border-radius: 16px; overflow: hidden; transition: all 0.3s;"
                                    :style="resolution === 'incoming' ? 'border-color: #10b981;' : 'border-color: #e2e8f0;'">
                                    <div style="padding: 16px 20px; border-bottom: 1px solid #f1f5f9; background: #fcfcfc; display: flex; justify-content: space-between; align-items: center;">
                                        <div style="display: flex; align-items: center; gap: 10px;">
                                        ...
                                        <div x-show="resolution === 'incoming'" style="background: #10b981; color: white; border-radius: 50%; padding: 4px;"><svg style="width: 12px; height: 12px;" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg></div>
                                    </div>
                                    <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
                                        <thead>
                                            <tr style="background: #f8fafc; border-bottom: 1px solid #f1f5f9;">
                                    ...
                                    <div style="padding: 24px; border-top: 1px solid #f1f5f9; background: #fafafa;">
                                        <button @click="isProcessing = true; setTimeout(() => { resolve('incoming'); isProcessing = false; }, 400)" style="width: 100%; padding: 14px; background: #064e3b; border: none; border-radius: 12px; color: white; font-size: 14px; font-weight: 800; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; box-shadow: 0 4px 6px -1px rgba(6, 78, 59, 0.4);">
                    ...
                    <div style="padding: 20px 32px; border-top: 1px solid #f1f5f9; background: white; display: flex; justify-content: space-between; align-items: center;">
                        <button @click="if(activeIndex > 0) activeIndex--" :disabled="activeIndex === 0" style="background: none; border: none; font-size: 14px; font-weight: 800; color: #1e293b; display: flex; align-items: center; gap: 8px; cursor: pointer; opacity: activeIndex === 0 ? 0.3 : 1;">
                        ...
                        <div style="display: flex; gap: 12px; align-items: center;">
                            <template x-if="resolved.filter(r => r === null).length === 0">
                                <form action="{{ route('suppliers.import.execute', $supplier) }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="strategy" value="manual">
                                    <template x-for="(conflict, index) in conflicts">
                                        <template x-for="lconflict in conflict.layer_conflicts">
                                            <input type="hidden" :name="'resolutions[' + conflict.layup_id + '_' + lconflict.layer_order + ']'" :value="resolved[index]">
                                        </template>
                                    </template>
                                    <button type="submit" style="background: #064e3b; color: white; padding: 10px 24px; border-radius: 30px; border: none; font-size: 13px; font-weight: 800; cursor: pointer; display: flex; align-items: center; gap: 8px;">
                                        Finalize Import
                            ...
                            <button @click="if(activeIndex < conflicts.length - 1) activeIndex++" :disabled="activeIndex === conflicts.length - 1" style="background: none; border: none; font-size: 14px; font-weight: 800; color: #1e293b; display: flex; align-items: center; gap: 8px; cursor: pointer; opacity: activeIndex === conflicts.length - 1 ? 0.3 : 1;">
                                Next Conflict
